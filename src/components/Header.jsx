  ...
  return (
    <div id="inicio" className="header bg-ise-gray-10 containerFull selection:bg-ise-orange selection:text-ise-white">
      <div id="header" className="containerContent flex-row justify-between p-3 items-start md:items-center lg:py-6 lg:px-4">
        ...
        <button type="button" onKeyUp={(e) => enterHandlesMenu(e)} onBlur={(e) => blurHandler(e)} className={`cursor-default lg:hidden absolute right-3 top-3 transition-all duration-1000 ${menu ? 'text-ise-orange' : 'flex gap-8 bg-ise-orange py-1 pr-1 pl-3 rounded-lg text-ise-white font-semibold'}`} data-testid={`menuButton ${!menu}`}>
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" onClick={() => handleClick()} className={`hover:cursor-pointer ${menu ? 'h-8 w-8' : 'h-6 w-6'}`} data-testid={`svgButton ${!menu}`}>
